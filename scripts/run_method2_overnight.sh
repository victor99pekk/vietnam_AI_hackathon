...
RUN_ROOT="${RUN_ROOT:-output_eval/overnight}"
RUN_ID="${RUN_ID:-$(date '+%Y%m%d_%H%M%S')}"
RUN_DIR="$RUN_ROOT/$RUN_ID"

if [[ -z "$KG_PATH" ]]; then
  echo "Usage: $0 PATH/TO/knowledge_graph.json"
  echo "Optional: DEVICE=cuda RUN_ROOT=output_eval/overnight nohup $0 ... &"
  exit 2
fi
...
# Keep a macOS workstation awake while the unattended experiment is running.
...
for index in "${!MODEL_NAMES[@]}"; do
  ...
  echo "[$(date '+%F %T')] Starting full A/B/C ablation: $model"
  command=(
...
if [[ "$all_ok" == true ]]; then
  ...
  echo "All overnight experiments completed: $RUN_DIR"
  exit 0
fi
